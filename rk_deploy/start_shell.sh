...
function set_nameserver_data() {
    ...
    SHELL_IP=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' $SHELL_ID)
...
function build_shell_image(){
    echo "building spark shell image"
    cd ../spark-$SPARK_VERSION/spark-shell
    . build
    cd -
    echo "build spark shell image success"
}

# starts the spark/shark shell container
function start_shell() {
    IMAGENAME="$image_name:$image_version"
    NAMESERVER_IP=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' $NAMESERVER_ID)
    echo "NAMESERVER_ID: $NAMESERVER_ID"
    echo "NAMESERVER_IP: $NAMESERVER_IP"
